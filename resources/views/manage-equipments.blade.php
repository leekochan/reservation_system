            ...
            @foreach($equipments as $equipment)
                <div x-data="{ showModal: false }">
                    <div class="bg-gray-100 rounded-lg shadow p-4 flex flex-col justify-between h-full">
                        ...
                        <p class="mt-4 text-start font-medium text-lg">{{ $equipment->equipment_name }}</p>

                        <div class="flex justify-end align-middle mt-4 gap-2">
                            <button @click="showModal = true">
                                <img src="{{ asset('pictures/icons/edit.png') }}" alt="Edit" class="w-5 h-5">
                            </button>
                            {{--                        <form action="{{ route('equipments.destroy', $equipment->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete it?')">--}}
                            <form method="POST" onsubmit="return confirm('Are you sure you want to delete it?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit">
                                    <img src="{{ asset('pictures/icons/delete.png') }}" alt="Delete" class="w-5 h-5 mt-2">
                                </button>
                            </form>
                        </div>
                    </div>
                    ...
                    <div x-show="showModal" x-cloak class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                        <div class="bg-white rounded-lg p-6 w-[90%] max-w-md relative border-2 border-[#7A1D30]">
                            ...
                            <form method="POST" enctype="multipart/form-data">
                                ...
                                <input type="text" name="equipment_name" value="{{ $equipment->equipment_name }}" class="w-full border px-3 py-2 mb-4 rounded">
                                ...
                                <div class="w-full h-40 bg-gray-100 border rounded flex items-center justify-center cursor-pointer mb-4">
                                    <label for="imageInput{{ $equipment->id }}" class="cursor-pointer text-gray-500 text-sm text-center w-full h-full flex flex-col items-center justify-center">
                                        <img src="{{ asset('pictures/icons/image-icon.png') }}" alt="Upload" class="w-12 h-12 mb-2">
                                        Upload from a computer
                                    </label>
                                    <input type="file" name="picture" id="imageInput{{ $equipment->id }}" class="hidden">
                                </div>
        ...
        <div x-show="showAddModal" x-cloak class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white rounded-lg p-6 w-[90%] max-w-md relative border-2 border-[#7A1D30]">
                ...
                <form method="POST" action="/admin/equipments" enctype="multipart/form-data">
                    ...
                    <input type="text" name="equipment_name" class="w-full border px-3 py-2 mb-4 rounded" required>
                    ...
                    <div class="text-center">
                        <button type="submit" class="bg-[#7A1D30] hover:bg-[#5c1524] text-white px-5 py-2 rounded">Add Equipment</button>
                    </div>
                </form>
            </div>
        </div>
